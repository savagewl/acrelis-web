...
// Общий CTA-блок с калькулятором — 1:1 совпадает по контенту на /automation (Figma
// id=719:757) и /it-solutions (Figma id=719:794), поэтому вынесен в общий компонент,
// а не задублирован. Иллюстрация графика — настоящий SVG-экспорт узла "Group" (719:777),
// а не растровый скриншот: у get_screenshot нет способа запросить рендер с большей
// плотностью пикселей, чем нативный размер узла (517×281) — на retina это давало
// заметную пикселизацию. Маска узла (719:775 "a") — просто прямоугольник на весь канвас
// без собственной формы, поэтому CSS-маска не нужна: сам SVG уже содержит и вертикальные
// линии сетки, и диагональную линию, и точки.
